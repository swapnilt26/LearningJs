...
addTranscationRows(movements);

// Total Balance
const totalSummary = function (movs) {
  return movs.reduce((totalBalance, element) => totalBalance + element);
};
labelBalance.innerText = `${totalSummary(movements)}€`;

// Total Deposit Balance
const totalInSummary = function (movs) {
  return movs
    .filter(e => e > 0)
    .reduce((transcationTotal, element) => transcationTotal + element, 0);
};
labelSumIn.innerHTML = `${totalInSummary(movements)}€`;

// Total Withdrawl Balance
const totalOutSummary = function (movs) {
  return Math.abs(
    movs
      .filter(e => e < 0)
      .reduce((transcationTotal, element) => transcationTotal + element)
  );
};
labelSumOut.innerHTML = `${totalOutSummary(movements)}€`;

// Add UserName
const addUserName = function (account) {
  account.userName = account.owner
    .split(' ')
    .map(e => e.at(0).toLowerCase())
    .join('');
};
addUserName(account4);
